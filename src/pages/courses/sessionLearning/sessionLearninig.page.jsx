      ...
      <div className="mt-3 d-flex flex-column justify-content-start align-items-stretch">
        {currentLesson ? (
          <CourseVideoItem
            ...
            set_sessionLessons={set_sessionLessons}
          >
          ...
          {sessionLessons?.length > 0 &&
            sessionLessons?.map((item, index) => (
              <Button
                ...
                variant="outlined"
                color={item.isSeen ? "success" : "primary"}
                onClick={() => {
                  gotoToSlide(index);
                }}
              >
                <span className="m-0 p-0 d-flex flex-row justify-content-start align-items-center">
                  {item?.isLocked && (
                    <LockIcon color={item.isSeen ? "success" : "primary"} />
                  )}
                  {!item?.isLocked && (
                    <LockOpenIcon color={item.isSeen ? "success" : "primary"} />
                  )}
                  <span className="ms-2">{item?.title}</span>
                </span>
                <span className="d-flex align-items-center">
                  {item.isSeen && (
                    <CheckIcon color="success" className="me-2" />
                  )}
                  {`${index + 1}/${sessionLessons?.length}`}
                </span>
              </Button>
            ))}
        </div>
