...
import AddToLeitner, {
  useHandleAddToLeitner,
} from "../../../components/toolbar/components/addToLeitner.component";
...
const Vocabularies = (props) => {
  ...
  const { handleAddToLeitner } = useHandleAddToLeitner({
    front: vocabularies[swiper?.activeIndex]?.phrase,
    back: vocabularies[swiper?.activeIndex]?.meaning,
    imageUrl: vocabularies[swiper?.activeIndex]?.imageUrl,
    voiceUrl: vocabularies[swiper?.activeIndex]?.voiceUrl,
    isVocab: true,
    onStart: handleOpen,
    onEnd: handleClose,
    onSuccess: (resp) => {
        ...
        <div className="d-flex flex-row justify-content-between align-items-stretch">
          {/**************************************/}
          {
            ...
            vocabularies?.length === counter && (
              ...
              >
                <Tooltip
                  title="برای تولید سوالات 4 گزینه ای حداقل به 4 لغت نیاز است."
                  placement="top"
                >
                  <div
                    ...
                    style={{
                      backgroundColor:
                        vocabularies?.length < 4 ? "#ccc" : "#228be6",
                      borderRadius: "10px",
                    }}
                  >
                    مشاهده سوالات
