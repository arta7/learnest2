...
const DetailCardItem = ({ data, title, cardIndex }) => {
  ...
  //
  // Card Content Dialog
  const [openCardContentDialog, set_openCardContentDialog] = useState(false);
  const toggle_openCardContentDialog = () => {
    set_openCardContentDialog(!openCardContentDialog);
  };
  //
  // Handle Card Click
  ...
  return (
    <>
      ...
      {/* ********* show content ********* */}
      {data[0]?.content &&
        data[0]?.content?.toString()?.trim() !== "" &&
        data[0]?.content?.toString()?.trim() !== "<p></p>" &&
        data[0]?.content?.toString()?.trim() !== "<p><br/></p>" && (
          <Dialog
            fullScreen
            open={openCardContentDialog}
            onClose={toggle_openCardContentDialog}
            TransitionComponent={Transition}
          >
            <div className="m-0 p-3 pb-0 hidden-scrollbar w-100 h-100 d-flex flex-column justify-content-start align-items-stretch">
              <Button
                variant="outlined"
                onClick={toggle_openCardContentDialog}
                className="align-self-end"
                size="sm"
              >
                <Close fontSize="md" />
              </Button>
              <h2 className="fs-5 fw-bold align-self-center my-3">توضیحات</h2>
              <div
                dir="auto"
              ...
              />
              <div
                className="mt-auto py-3 w-100 d-flex flex-row justify-content-center align-items-center bg-white"
                style={{
                  position: "sticky",
                  bottom: "0",
                  left: "0",
              ...
              >
                <Button
                  ...
                  color="primary"
                  onClick={handleClick}
                  className="w-50 py-2 mx-auto mb-0"
                >
                  ادامه
                </Button>
              </div>
