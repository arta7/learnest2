...
const CoursesDetails = () => {
  ...
  ////// GOTO LEVEL DETERMINE //////
  const [isFetcingCourseDetails, set_isFetcingCourseDetails] = useState(false);
  const getCourseDetails = async () => {
    handleOpen();
    set_isFetcingCourseDetails(true);
    await axios
      ...
      .finally(() => {
        handleClose();
        set_isFetcingCourseDetails(false);
      });
  };
  ...
  return (
    <section style={{ position: "relative", maxWidth: "800px" }}>
      <CourseDetailsHeader
        ...
        headerData={courseDetails?.header}
      />
      <CourseDetailsTab
        detailsData={courseDetails}
        isFetcingCourseDetails={isFetcingCourseDetails}
      />
      ...
      {(courseDetails?.introduction?.isBought === false ||
        ...
        >
          ...
          {courseDetails?.introduction?.isBought === true &&
          courseDetails?.introduction?.examScore > 0
            ? `امتیاز شما در آزمون این دوره : ${
                parseFloat(courseDetails?.introduction?.examScore) * 100
              } درصد`
            : ""}
        </div>
      )}
      ...
      >
        <div
          style={{ minWidth: "250px", minHeight: "350px" }}
