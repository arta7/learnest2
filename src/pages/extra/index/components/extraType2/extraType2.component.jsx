...
const ExtraType2 = ({
  ...
  //
  const onExtraCompeleted = async () => {
    await apiCaller({
      api: extra_apiCalls.apiCall_completeExtra,
      apiArguments: {
        id: id,
        extraType: extraDetails?.extraMediaType,
        percent: 100,
      },
    });
  };
  const handleSeeVocabs = async () => {
  ...
  return (
    <div
      ...
      className={
        (isCompleted ? "border border-success" : "") +
        " m-0 m-0 p-0 shadow box-rounded-1 cursor-pointer d-flex flex-row justify-content-start align-items-stretch"
      }
    >
