...
const ExtraDetailsPage = () => {
  ...
  //
  const onExtraCompeleted = () => {
    apiCaller({
      api: extra_apiCalls.apiCall_completeExtra,
      apiArguments: {
        id: parse(window.location.search).id,
        extraType: location.state.extraType,
        percent: 100,
      },
    });
  };
  const [intervalId, set_intervalId] = useState();
  const [secondsPassed, set_secondsPassed] = useState(0);

  useEffect(() => {
    const interval = setInterval(() => {
      set_secondsPassed((secondsPassed) => secondsPassed + 1);
    }, 1000);
    set_intervalId(interval);
  }, []);
