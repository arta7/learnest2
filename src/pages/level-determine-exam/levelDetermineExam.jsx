...
const LevelDetermineExam = (props) => {
  ...
  /////// check is last exam
  const getIsLastExam = () => {
    if (location.state.hasOwnProperty("isLastExam")) {
      if (location.state?.isLastExam) {
        return true;
      }
      return false;
    }
          ...
          <div className="exam-finished d-flex flex-column justify-content-start align-items-stretch">
            {examFinishState && (
              <>
                ...
                >{` وضعیت آزمون : ${examFinishState?.testStatusString}`}</span>
                <div
                ...
                {examFinishState?.testStatus !== 3 && !getIsLastExam() && (
                  <>
                    <div className="d-flex flex-row justify-content-center align-items-center">
                      <Button
                        variant="contained"
