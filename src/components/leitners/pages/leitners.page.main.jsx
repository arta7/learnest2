...
const Leitner = (props) => {
  ...
  const updateLiernerItem = async (currentItemNewLevel) => {
    handleOpen();
  ...
  const handleSetLevel = (level) => {
    ...
    if (currentItem.box + level > 4) {
      clonedLietnerItems[index].box = 5;
    ...
    if (
      ...
      if (currentItem.box === 4 && level > 0) {
        updateLiernerItem(5);
      }
      if (currentItem.box === 0 && level < 0) {
        updateLiernerItem(0);
      }
    ...
    updateLiernerItem(clonedLietnerItems[index].box);
    ...
    <section className="m-0 p-0 ">
      ...
      {leitnerItems?.length > 0 && itemsEnded && (
        <div
          ...
          className="m-0 p-3 text-justify fs-5 w-100 text-success d-flex flex-column justify-content-center align-items-center"
        >
          دوست لرنستی عزیز ، شما امروز تمام کارت های موجود در جعبه‌ی لایتنر را
          مرور کرده اید. در صورتی که امروز کارت جدیدی اضافه کنید میتوانید مجدد
          آنرا مشاهده نمایید و در غیر این صورت برای ادامه‌ی مرور کارتهای دیده
          شده در جعبه های لایتنر تا هنگام فراگیری کامل و خروج آنها از هر پنج
          جعبه‌ی لایتنر ، فردا مجددا وارد این بخش شوید .
        </div>
      )}
      ...
      {!isLoading && leitnerItems?.length === 0 && (
        <div
          ...
          className="m-0 p-3 text-justify fs-5 w-100 d-flex flex-column justify-content-center align-items-center"
        >
          دوست لرنستی عزیز ، شما تمام کارت های موجود در جعبه‌ی لایتنر را مرور
          کرده اید. در صورتی که امروز کارت جدیدی اضافه کنید میتوانید مجدد آنرا
          مشاهده نمایید و در غیر این صورت برای ادامه‌ی مرور کارتهای دیده شده در
          جعبه های لایتنر تا هنگام فراگیری کامل و خروج آنها از هر پنج جعبه‌ی
          لایتنر ، فردا مجددا وارد این بخش شوید .
        </div>
      )}
    </section>
  );
