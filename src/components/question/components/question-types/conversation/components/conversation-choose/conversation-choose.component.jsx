...
const ConversationChoose = ({
  ...
  const [currentQuestion, set_currentQuestion] = useState();
  ...
  useEffect(() => {
    ...
    /// Select First Question For User
    const questionsWithField = Data.data?.filter((item) =>
      item.body.includes("##")
    );
    if (questionsWithField && questionsWithField?.length > 0) {
      set_currentQuestion(questionsWithField[0]);
    }
    // console.log(arr);
  }, []);
  ...
  const handle_selectAnswer = (option) => {
    // check currentQuestion has field
    if (!currentQuestion.body.includes("##")) {
      return;
    }
...
export default ConversationChoose;
