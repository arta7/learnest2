...
import "./ynb-chooseRepeat.styles.scss";
...
const YnbChooseRepeat = ({
  ...
  const handle_clearAnswerSheet = () => {
    ...
    set_currentQuestion(Data.data[0]);
  };
    ...
    <div className="ynb-text-content mb-6">
        ...
        <div className="ynb-answer mt-3">
          <AnsweringPart
            ...
            seetrueanswerprops={{
              ...
              enableOptionalButtons: enableOptionalButtons,
            }}
          />
        </div>
      )}
      {answerSheet?.length > 0 && !showSeeResult && !Data?.mixedAnswers && (
        <Button
          ...
          color="primary"
          className="ms-2"
          onClick={handle_clearAnswerSheet}
        >
          پاک کردن همه
        </Button>
      )}
      {showSeeResult && (
        <div className="see-result cursor-pointer justify-self-center mx-auto text-center mt-4  d-flex flex-row justify-content-center">
          <SeeResult
            isLastQuestion={isLastQuestion}
            hiddenContent={Data?.hiddenContent}
...
export default YnbChooseRepeat;

/*

 const [selectedAnswers, set_selectedAnswers] = useState([]);
    const handle_select_answer = (option) => {
        ...
        if (selectedAnswers.length > 0) {
            ...
            set_selectedAnswers(newAnswers);
        }
        else {
            set_selectedAnswers([option]);
        }
    };


    return (
        <div className='ynb-image-content'>
            <div dir='ltr' className='my-2 ynb-q'>
                {Data.data.map(q => (
                    <div key={q.id} className=''>
                        {q.body.toString().replaceAll("##","________")}
                    </div>
                ))}
            </div>
            <div className='ynb-answer-1 d-flex flex-row justify-content-between align-items-stretch flex-wrap'>
                {
                    Data.options.map((option) => (
                        <span key={option} className={(selectedAnswers.indexOf(option) > -1 ? "bg-success" : "bg-danger") + ' border p-2 cursor-pointer'} onClick={() => { handle_select_answer(option); }}>{option}</span>
                    ))
                }
            </div>
        </div>
