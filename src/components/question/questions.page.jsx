...
const QuestionsIterator = () => {
      ...
      const newData = currentQuestionClone?.data?.data.map((item, index) => {
        ...
        if (contentType === 0) {
          if (emptyFields === 1) {
            ...
            /// *** this snippet is for comparing answers behind thhe sence
            let uab_forCompare = questionBody;
            let cab_forCompare = questionBody;
            /// *** this snippet is for showing to userlet uab = questionBody;
            let uab_forShow = questionBody;
            let cab_forShow = questionBody;
            ///// LOOPING OVER THE ANSWERS
            for (let i = 0; i < emptyFields - 1; i++) {
              uab_forCompare = uab_forCompare.replace(
                "##",
                mixedAnswers[index].userAnswer.options[i]
              );
              uab_forShow = uab_forShow.replace(
                "##",
                "<b>" + mixedAnswers[index].userAnswer.options[i] + "</b>"
              );
              cab_forCompare = cab_forCompare.replace(
                "##",
                mixedAnswers[index].correctAnswer.options[i]
              );
              cab_forShow = cab_forShow.replace(
                "##",
                "<b>" + mixedAnswers[index].correctAnswer.options[i] + "</b>"
              );
            }

            mixedBody.userAnswerBody = uab_forShow;
            mixedBody.correctAnswerBody = cab_forShow;
            mixedBody.isRight =
              uab_forCompare.toLowerCase() === cab_forCompare.toLowerCase();
          }
        } else {
          mixedBody.isRight =
  ...
  const restartExam = async () => {
    ...
    apiCaller({
      ...
      apiArguments:
        sessionLearningData?.pageDataOnFinishQuestions?.data?.courseId,
      onErrorMessage: "دریافت اطلاعات آزمون با خطا مواجه شد .",
      onError: handleClose,
      onSuccess: (resp) => {
        // changeTypeId(resp?.data?.data?.exam?.id);
        apiCaller({
          api: exam_apiCalls.apiCall_startExam,
          apiArguments: resp?.data?.data?.id,
          ...
          onSuccess: (response) => {
            if (sessionLearningData?.questions?.length > 0) {
              ...
              setTimeout(() => {
                set_questions(response?.data?.data?.questions);
                set_curentQuestion(response?.data?.data?.questions?.[0]);
                set_responseTime(response?.data?.data?.responseTime);
                set_typeId(resp?.data?.data?.id);
              }, 200);
            } else {
              navigate(-1);
            }
